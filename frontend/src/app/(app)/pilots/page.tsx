...
function CertificationList({ pilotId }: { pilotId: string }) {
  const { data, isLoading } = useCertifications(pilotId);
...
export default function PilotsPage() {
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader title="Pilotët" count={pilots.length}>
        {expiring && expiring.length > 0 && (
          <div className="flex items-center gap-2 text-[var(--status-caution)]">
            <FileWarning className="h-3.5 w-3.5" strokeWidth={2} />
            <span className="text-[13px]">
              {expiring.length} certifikata skadojnë brenda{" "}
              {THRESHOLDS.certificationExpiryDays} ditësh
            </span>
          </div>
        )}
      </PageHeader>
            ...
            <>
              ...
              <div className="mb-8 grid grid-cols-3 gap-4">
                {[
                  ...
                  {
                    label: "Skadojnë",
                    value: selected.expiringCertificationCount,
                  },
                ].map((m) => (
